...
void Quick_Sort(pair<int,int>* arr,int l,int r){
    ...
    mid= arr[(left+right)/2].first;
    int mid_second=arr[(left+right)/2].second;
    while(right>=left){
        if(mid<=arr[left].first && mid>=arr[right].first){
        ...
        if(mid>arr[left].first ||(mid==arr[left].first &&( mid_second>=arr[left].second))){
            left++;
        }
        if(mid<arr[right].first ||(mid==arr[right].first &&( mid_second<=arr[right].second))){
            right--;
        }
